 ...
 *
 *  `/v1/vehicles` carries a caveat. With the legacy measurement net paused, a
 *  request that carries a bounding box fetches live positions ON DEMAND from
 *  mgate. That is a real upstream call — not against the VAO contract allowance,
 *  but against its own daily cap (2500/day, 15 s TTL, single-flight per
 *  viewport). The bbox below is therefore FIXED and identical for every virtual
 *  user on every machine, so the whole distributed run coalesces into roughly
 *  one upstream call per 15 s window no matter how many requests it sends.
 *  Randomising this bbox would defeat the coalescing and spend the cap. */
...
export function assertSafe(paths) {
